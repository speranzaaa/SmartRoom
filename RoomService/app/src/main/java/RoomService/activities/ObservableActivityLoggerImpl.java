...
/**
 * An ObservableActivityLogger wrap an ActivityLogger so every new activity can be observed.
 * <b>
 ...
 */
public class ObservableActivityLoggerImpl implements ObservableActivityLogger {
	
	private final ActivityLogger activityLogger;
	private final Set<Consumer<Activity>> listeners;
	
	/**
	 * Create a new ObservableActivityLogger wrapping an existing ActivityLogger.
	 * @param activityLoggerToObserve The activity logger to wrap and observe.
	 */
	public ObservableActivityLoggerImpl(ActivityLogger activityLoggerToObserve) {
		this.activityLogger = activityLoggerToObserve;
		this.listeners = new HashSet<>();
	}

	@Override
	public void logActivity(final Activity activity) {
		this.activityLogger.logActivity(activity);
		this.listeners.stream().forEach(listener->listener.accept(activity));
	}

	@Override
	public List<Activity> getActivities() {
		return this.activityLogger.getActivities();
	}

	@Override
	public void clearBefore(Date date) {
		this.activityLogger.clearBefore(date);
	}
